...
var builtinRules = []builtinRule{
	...
	// ---------------- Rust ----------------
	rule("builtin.command-injection.rust", "high",
		"Command injection: std::process::Command runs a shell (-c) with a command string built by format!/concat! or concatenation.",
		"Avoid shell invocation entirely: pass arguments as separate Command args, never as a single -c shell string; validate any input that reaches a shell.",
		exts(".rs"),
		`Command\s*::\s*new\s*\(`,
		`(-c\s*["',]|format!\s*\(|concat!\s*\(|\s*\+)`, "", ""),
	rule("builtin.sql-injection.rust", "high",
		"SQL injection: a rusqlite/sqlx query is built with format!/concat! instead of bound parameters.",
		"Use bound parameters (query with params![]) and never interpolate input into SQL text.",
		exts(".rs"),
		`\.(query|query_row|query_map|query_named|execute|execute_batch|fetch_all|fetch_one)\s*\(`,
		`(format!\s*\(|concat!\s*\(|\s*\+)`,
		`(?i)\b(SELECT|INSERT|UPDATE|DELETE|DROP)\b`, ""),
	rule("builtin.weak-crypto.rust", "medium",
		"Weak cryptography: the md5/sha1 crates (or Md5/Sha1 types) are cryptographically broken.",
		"Use a modern crate (sha2, argon2, or the RustCrypto AEADs) instead of md5/sha1 for security purposes.",
		exts(".rs"),
		`\b(md5|sha1)\s*::|\b(Md5|Sha1)\b`, "", "", ""),
	rule("builtin.unsafe-memory.rust", "medium",
		"Unsafe memory operation: transmute / from_utf8_unchecked / MaybeUninit::uninit on data whose provenance is not guaranteed.",
		"Prefer safe conversions (bytemuck, From/Into, checked UTF-8 parsing); if unsafe is unavoidable, document and verify the invariants.",
		exts(".rs"),
		`(std::mem::transmute|from_utf8_unchecked|MaybeUninit::uninit)`, "", "", ""),
	rule("builtin.path-traversal.rust", "medium",
		"Path traversal: a file path is built with format!/concat! from dynamic values instead of being validated.",
		"Validate/normalize user-controlled path components, reject traversal sequences (..), and prefer safe APIs like PathBuf with a base dir check.",
		exts(".rs"),
		`(fs::(read|read_to_string|write|read_dir|metadata)|File::(open|create))\s*\(`,
		`(format!\s*\(|concat!\s*\(|\s*\+)`, "", ""),
	...
	// ---------------- JavaScript / TypeScript: SSRF, path traversal, prototype pollution ----------------
	rule("builtin.ssrf.javascript", "medium",
		"Server-Side Request Forgery: an outbound HTTP request targets a URL assembled from untrusted data.",
		"Validate and allowlist the target host/scheme; never let user input select an arbitrary (internal) URL.",
		exts(".js", ".jsx", ".ts", ".tsx"),
		`(fetch\s*\(|axios\.(get|post|put|delete|request)\s*\(|http\.(get|request)\s*\(|https\.(get|request)\s*\()`,
		`(\$\{|req\.|query\.|params\.|body\.|headers\.|url\s*=\s*)`, "", ""),
	rule("builtin.path-traversal.javascript", "medium",
		"Path traversal: a file path is built from user-controlled values (req.params/query/body) without validation.",
		"Validate and normalize user-controlled path segments, reject '..' and absolute paths, and resolve the final path against a whitelisted base directory.",
		exts(".js", ".jsx", ".ts", ".tsx"),
		`(fs\.(readFile|readFileSync|writeFile|writeFileSync|readdir|readdirSync|createReadStream|createWriteStream|unlink|unlinkSync|stat|statSync)\s*\(|path\.join\s*\()`,
		`(\$\{|req\.|query\.|params\.|body\.)`, "", ""),
	rule("builtin.prototype-pollution.javascript", "medium",
		"Prototype pollution: an assignment writes into __proto__ / constructor.prototype from dynamic keys.",
		"Use Object.create(null) for untrusted-key dictionaries, freeze prototypes, and reject '__proto__'/'constructor' keys in recursive merges.",
		exts(".js", ".jsx", ".ts", ".tsx"),
		`(\[['"]__proto__['"]\]|\.__proto__\s*=|constructor\.prototype)`,
		`(\[key\]|\[prop\]|\[name\]|assign\s*\(|merge\s*\(|\$\{|req\.)`, "", ""),

	// ---------------- Python: SSRF, path traversal, XXE ----------------
	rule("builtin.ssrf.python", "medium",
		"Server-Side Request Forgery: an outbound request targets a URL assembled from untrusted data.",
		"Allowlist target hosts/schemes and never let user input select an arbitrary internal URL.",
		exts(".py"),
		`\b(requests\.(get|post|put|delete|patch|request)|urllib\.request\.urlopen|aiohttp\.ClientSession|httpx\.(get|post|put|delete|request))\s*\(`,
		`(request|input\s*\(|sys\.argv|url\s*=|os\.environ|open\s*\()`, "", ""),
	rule("builtin.path-traversal.python", "medium",
		"Path traversal: a file path is assembled from user-controlled input without sanitization.",
		"Validate/normalize user-controlled path components and resolve paths against a base directory with a containment check.",
		exts(".py"),
		`\bopen\s*\(|os\.(remove|rename|makedirs|listdir|path\.join|stat)\s*\(`,
		`(request|input\s*\(|sys\.argv|filename|filepath|path\s*=)`, "", ""),
	rule("builtin.xxe.python", "medium",
		"XML External Entity: lxml/etree is configured to resolve external entities, enabling XXE on untrusted XML.",
		"Keep resolve_entities=False (the default) and no_network=True when parsing untrusted XML.",
		exts(".py"),
		`(lxml|etree)\.(fromstring|parse)\s*\(|XMLParser\s*\(`,
		`resolve_entities\s*=\s*True|no_network\s*=\s*False`, "", ""),

	// ---------------- Go: SSRF, path traversal, weak randomness ----------------
	rule("builtin.ssrf.go", "medium",
		"Server-Side Request Forgery: an HTTP request is sent to a URL assembled from concatenated/unvalidated data.",
		"Validate and allowlist the target host/scheme; never send requests to attacker-chosen URLs.",
		exts(".go"),
		`\b(http\.(Get|Post|NewRequest)|client\.(Get|Post|Do)|url\.Parse)\s*\(`,
		`(\+|fmt\.Sprintf\s*\(|os\.Args|r\.(URL|FormValue|Query)|req\.)`, "", ""),
	rule("builtin.path-traversal.go", "medium",
		"Path traversal: a file path is assembled from concatenated values without a base-dir containment check.",
		"Resolve the final path and verify it stays inside the intended base directory; reject '..' segments.",
		exts(".go"),
		`(os\.(Open|ReadFile|WriteFile|Create|ReadDir|Stat|Remove)\s*\(|filepath\.Join\s*\(|http\.Dir\s*\(|http\.FileServer\s*\()`,
		`(\+|fmt\.Sprintf\s*\(|r\.(URL|FormValue|Query)|req\.|os\.Args)`, "", ""),
	rule("builtin.weak-random.go", "medium",
		"Weak randomness: math/rand (non-cryptographic) is used where an unpredictable value is expected.",
		"Use crypto/rand for tokens, IDs, salts and anything security-sensitive.",
		exts(".go"),
		`\bmath/rand\b|rand\.(Int|Intn|Int63|Float|Perm|Read)\s*\(`,
		`(?i)(token|secret|salt|password|nonce|session|csrf|verify|api.?key)`, "", ""),

	// ---------------- Java: XXE, path traversal, LDAP injection ----------------
	rule("builtin.xxe.java", "high",
		"XML External Entity: DocumentBuilderFactory/SAXParserFactory/XMLInputFactory is configured with external DTD/entity expansion enabled — XXE on untrusted XML.",
		"Disable DOCTYPE and external entities (disallow-doctype-decl=true, disable external-general/parameter entities) on every XML parser.",
		exts(".java"),
		`(DocumentBuilderFactory\.newInstance|SAXParserFactory\.newInstance|XMLInputFactory\.newFactory)`,
		`(setFeature|setAttribute|XMLConstants)`, "", ""),
	rule("builtin.path-traversal.java", "medium",
		"Path traversal: a file path is built from request parameters without validation.",
		"Validate request parameters, reject '..' segments, and resolve paths against a whitelisted base directory.",
		exts(".java"),
		`(new\s+File\s*\(|Files\.(readString|readAllBytes|write|newBufferedReader|newInputStream|copy|delete|exists)\s*\(|Paths\.get\s*\()`,
		`(getParameter|getHeader|getPathInfo|getRequestURI|request\.)`, "", ""),
	rule("builtin.ldap-injection.java", "medium",
		"LDAP injection: a search filter is assembled by concatenating request data.",
		"Escape filter values per RFC 4515 or use an allowlist; never concatenate input into filters.",
		exts(".java"),
		`(InitialDirContext|DirContext|LdapContext|LdapName|search\s*\()`,
		`(\s*\+|getParameter|getHeader)`, "", ""),

	// ---------------- C#: SSRF, path traversal, weak crypto ----------------
	rule("builtin.ssrf.csharp", "medium",
		"Server-Side Request Forgery: HttpClient/WebClient makes a request to a URL assembled from request data.",
		"Allowlist target hosts and never pass user-controlled URLs to the client.",
		exts(".cs"),
		`\.(GetAsync|PostAsync|SendAsync|DownloadString|DownloadData|GetStringAsync|DownloadStringTaskAsync)\s*\(`,
		`(\s*\+|Request\.(QueryString|Form|Headers|Params)|[Aa]rguments)`, "", ""),
	rule("builtin.path-traversal.csharp", "medium",
		"Path traversal: a file path is built from request data without validation.",
		"Validate/normalize request-derived paths, reject '..' segments and constrain the result to a base directory.",
		exts(".cs"),
		`(File\.(ReadAllText|ReadAllBytes|WriteAllText|WriteAllBytes|OpenRead|OpenWrite|Exists|Delete)\s*\(|Path\.Combine\s*\(|new\s+FileInfo\s*\()`,
		`(\s*\+|Request\.(QueryString|Form|Headers|Params))`, "", ""),
	rule("builtin.weak-crypto.csharp", "medium",
		"Weak cryptography: MD5/SHA1 algorithms are cryptographically broken.",
		"Use SHA-256+ or AES-GCM from System.Security.Cryptography; never MD5/SHA1 for security.",
		exts(".cs"),
		`(?i)(MD5\.Create|SHA1\.Create|MD5CryptoServiceProvider|SHA1CryptoServiceProvider|new\s+MD5|new\s+SHA1)`, "", "", ""),

	// ---------------- PHP: weak crypto ----------------
	rule("builtin.weak-crypto.php", "medium",
		"Weak cryptography: md5()/sha1() are used for passwords or security-critical hashing.",
		"Use password_hash()/password_verify() (bcrypt/argon2) for passwords and sha256+ for hashing.",
		exts(".php"),
		`\b(md5|sha1)\s*\(`,
		`(?i)(password|pass|token|auth|secret)`, "", ""),

	// ---------------- Lua ----------------
	rule("builtin.command-injection.lua", "high",
		"Command injection: os.execute/io.popen runs a shell command built by concatenation.",
		"Pass arguments without a shell and validate any input that reaches a command.",
		exts(".lua"),
		`(os\.execute|io\.popen)\s*\(`,
		`(\s*\.\.|string\.format\s*\(|ngx\.var|ngx\.req|arg|request)`, "", ""),
	rule("builtin.sql-injection.lua", "high",
		"SQL injection: a database query is built by concatenating values instead of binding parameters.",
		"Use prepared statements with bound parameters (luasql/sqlite3 :prepare + bind).",
		exts(".lua"),
		`(db:execute|conn:execute|db:query|:query|:prepare)\s*\(|luasql\.\w+\s*\(`,
		`(\s*\.\.|string\.format\s*\()`, `(?i)\b(SELECT|INSERT|UPDATE|DELETE|DROP)\b`, ""),
	rule("builtin.code-execution.lua", "high",
		"Arbitrary code execution: loadstring/load compiles a string that may contain untrusted data.",
		"Remove dynamic code compilation; validate the data source if unavoidable.",
		exts(".lua"),
		`\b(loadstring|load)\s*\(`,
		`(\s*\.\.|ngx\.var|ngx\.req|arg|request)`, "", ""),

	// ---------------- PowerShell ----------------
	rule("builtin.code-execution.powershell", "high",
		"Arbitrary code execution: Invoke-Expression (IEX) executes a string, which is critical when it carries untrusted data.",
		"Remove IEX; use safe alternatives and never evaluate input-derived strings.",
		exts(".ps1"),
		`(?i)\b(Invoke-Expression|iex)\b\s+`, "", "", ""),

	// ---------------- XPath injection (generic) ----------------
	rule("builtin.xpath-injection", "medium",
		"XPath injection: an XPath expression is built by concatenating user input.",
		"Parameterize XPath queries (variables) or validate/escape input before use.",
		exts(".java", ".js", ".jsx", ".ts", ".tsx", ".py", ".cs", ".php"),
		`(xpath\.evaluate|XPathFactory|XPathExpression|SelectSingleNode|SelectNodes)\s*\(|\.xpath\s*\(`,
		`(\s*\+|#\{|\$\{|%s|\.format\s*\(|getParameter|request\.)`, "", ""),
...
var namedSecrets = []namedSecret{
	...
	{regexp.MustCompile(`\bglpat-[0-9A-Za-z_\-]{20,}\b`), "high", "GitLab personal access token hardcoded in source"},
	{regexp.MustCompile(`\bAC[0-9a-f]{32}\b`), "medium", "Twilio Account SID hardcoded in source"},
	{regexp.MustCompile(`(?i)\bSK[0-9a-f]{32}\b`), "high", "Twilio auth token hardcoded in source"},
	{regexp.MustCompile(`(?i)AccountKey\s*=\s*[A-Za-z0-9+/=]{40,}`), "high", "Azure Storage account key hardcoded in source"},
	{regexp.MustCompile(`(?i)\b(?:private[_-]?key)\s*[=:]\s*["']?[A-Za-z0-9_\-+/=]{16,}`), "medium", "Private key value hardcoded in source"},
}
